...
$(document).ready(() => {
  ...
  aux += `<img src="../resources/img/logos/${datosEditorial[0].logo}" width="240px" height="285px">`;
  ...
  aux += `<h4 class="text-white">${datosEditorial[0].nombreEditorial}</h4>`;
  ...
  for (let i = 0; i < comicsEditorial.length; i++) {
    ...
    aux2 += `<h4 class="card-title">${comicsEditorial[i].nombreComic}</h4>`;
    aux2 += `<a class="btn btn-primary align-items-end" href="#myModal" onClick="${modalDatosComic(comicsEditorial[i].idComic)}" data-toggle="modal" data-target=".bd-example-modal-lg">Ver detalles</a>`;
